...
pdf("~/Space-time-manuscript/S1 - Regression Plots/Plots_RO_APPENDIX1.pdf", height = 10)
...
for(i in 1:25){
  ...
  ci.time <- sapply(x.scaled,  function(x) PI(exp(draws$a[, i, 1] + draws$b_time[, i] * x + 
                                                    mean(draws$observer[, , 1]) + mean(draws$first[,1]) +
                                                    draws$retrans_noise )) )
  plot(d_filter$Forest.cover, d_filter$Richness_ALL, #d_filter$TA_ALL, 
...
setwd("~/Space-time-manuscript/")
pdf_convert("~/Space-time-manuscript/S1 - Regression Plots/Plots_RO_APPENDIX1.pdf",  pages = 1:4)
