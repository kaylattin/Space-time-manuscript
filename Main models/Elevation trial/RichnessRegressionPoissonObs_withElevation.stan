...
transformed parameters{
 ...
 vector[nfirstobs] first;
  ...
  lambda[i] = a[reg[i], spacetime[i]] + b_time[reg[i]] * time[i] * pforest[i] + b_space[reg[i]] * space[i] * pforest[i] + b_elev * space[i] * elevation[i] + observer[obs[i], spacetime[i]] + first[firstobs[i]] + noise;
...
generated quantities{
  ...
  retrans_noise = 0.5*(sdnoise^2);
    ...
    sim_space[i] = poisson_log_rng(avg_a_space + avg_b_space * xrep[i] + avg_obs_space + avg_first_obs + noise); //elevation removed; assume elevation is roughly the same for all landscapes in space to isolate effect of forest, Z-score = 0
