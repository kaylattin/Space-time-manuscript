...
### RICHNESS! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
d <- read.csv("FinalDataset_RFsub.csv")
...
Cairo(file="REVISED_Slopes_RF_bg_V2.png", 
      type="png",
      units="cm", 
      width=40, 
      height=30, 
...
for(i in 1:30){
  ...
  mu.time <- sapply(x.scaled,  function(x) mean(exp(draws$a[, i, 1] + draws$b_time[, i] * x + 
                                                      mean(draws$observer[, , 1]) + mean(draws$first[,2]) +
                                                      draws$retrans_noise )) )
...
png(file="REVISED_Slopes_RF_fg_V2.png", 
    type="cairo",
    units="cm", 
    width=40, 
    height=30, 
...
d <- read.csv("FinalDataset_TFsub.csv")
...
png(file="REVISED_Slopes_TF_fg.png", 
      type="cairo",
      units="cm", 
      width=40, 
      height=30, 
