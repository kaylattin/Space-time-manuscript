...
pdf("~/space-for-time/Revised Submission_Ecological Applications/Supplementary Plots/Plots_TO_REVISED.pdf", height = 10)
...
for(i in 1:30){
  ...
  mu.space <- sapply(x.scaled,  function(x) mean(exp(draws$a[, i, 2] + draws$b_space[, i] * x + 
                                                       mean(draws$observer[, , 2]) + mean(draws$first[,1:2]) + 
                                                       draws$retrans_noise ) ))
...
setwd("~/space-for-time/Revised Submission_Ecological Applications/Supplementary Plots/")
pdf_convert("~/space-for-time/Revised Submission_Ecological Applications/Supplementary Plots/Plots_TO_REVISED.pdf",  pages = 1:4)
